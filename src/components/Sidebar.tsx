  ...
  return (
    ...
    >
      ...
      <div className={`border-t border-slate-200/60 mt-2 mb-4 pt-3 ${isCollapsed ? 'px-1' : 'px-0'}`}>
         <div 
            onClick={onLogout}
            className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} p-2 rounded-lg hover:bg-white hover:shadow-sm transition-all cursor-pointer group`}
         >
            {/* Avatar */}
             ...
             {!isCollapsed && (
                 <div className="flex-1 min-w-0">
                     <div className="text-[14px] font-bold text-slate-900 truncate">{user?.email || 'User'}</div>
                     <div className="text-[12px] text-slate-500 truncate">Pro Member</div>
                 </div>
             )}
